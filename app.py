...
def add_admission_fees():
    if request.method == 'POST': 
        ...
        first_term_amount = float(request.form['first_term_amount'])
        second_term_amount = float(request.form['second_term_amount'])
        total_amount = admission_fees + first_term_amount + second_term_amount

        cursor = mysql.connection.cursor()
        cursor.execute(
            "INSERT INTO admission_fees (class_name, admission_fees, first_term_amount, second_term_amount, total_amount) VALUES (%s, %s, %s, %s, %s)", 
            (class_name, admission_fees, first_term_amount, second_term_amount, total_amount)
        )
        mysql.connection.commit()
        cursor.close()
...
def update_admission_fees():
    if 'logged_in' in session:
        admission_fees_id = request.form['admission_fees_id']
        admission_fees = request.form['admission_fees']
        first_term_amount = request.form['first_term_amount']
        second_term_amount = request.form['second_term_amount']
        total_amount = float(admission_fees) + float(first_term_amount) + float(second_term_amount)
        
        cursor = mysql.connection.cursor()
        cursor.execute("""
            UPDATE admission_fees 
            SET admission_fees=%s, first_term_amount=%s, second_term_amount=%s, total_amount=%s
            WHERE admission_fees_id=%s
        """, (admission_fees, first_term_amount, second_term_amount,total_amount, admission_fees_id))
        mysql.connection.commit()
        cursor.close()
        
        flash('Admission fees updated successfully!')
        return redirect(url_for('fees_structure'))
    else:
        flash('You need to be logged in to update admission fees.')
        return redirect(url_for('admin_login'))
...
def new_admission():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if not caste:
                flash('Please enter your caste.', 'danger')
                return redirect(url_for('new_admission'))
        
            if len(caste) < 2:
                flash('Caste must be at least 2 characters long.', 'danger')
                return redirect(url_for('new_admission'))
...
def submit_payment():
    ...
    rendered = render_template('pay_fees.html', student=student_data, fees=fees, second_term_pay=second_term_pay)
...
def contact():
    ...
    if request.method == 'POST':
        ...
        if not email:
            ...
            cursor = mysql.connection.cursor()
            cursor.execute('SELECT COUNT(*) FROM contact_requests WHERE email = %s', (email,))
